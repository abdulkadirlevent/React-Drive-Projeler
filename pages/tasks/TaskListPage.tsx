...
import { getTodos, deleteTodo, getProjects } from '../../data';
...
const TaskListPage: React.FC = () => {
    ...
    useEffect(() => {
        setTitle("Tüm Görevler");
        setPlaceholder("Görevlerde arayın...");
        setLoading(true);
        setTasks(getTodos());
    ...
    return (
        <div className="w-full">
            <div className="flex items-center justify-end mb-4">
                 <Link to="/tasks/new" className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-md">
            ...
            <div className="bg-white dark:bg-[#1f1f1f] rounded-lg shadow-md overflow-hidden">
                <table className="w-full table-auto">
                    ...
                    <tbody>
                        {currentItems.map((item: TodoItem) => (
                            <tr key={item.id} className="border-b border-gray-200 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-800/50 text-sm">
                                ...
                                <td className="px-4 py-3">
                                     <Link to={`/projects/${item.projectId}`} className="hover:underline text-blue-600 dark:text-blue-400">
                                        {projectMap.get(item.projectId) || 'Bilinmeyen Proje'}
                                    </Link>
                                </td>
                                <td className="px-4 py-3">
                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${item.completed ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200'}`}>
                                        {item.completed ? 'Tamamlandı' : 'Bekliyor'}
                                    </span>
                                </td>
                                 <td className="px-4 py-3 text-sm text-center">
                                    <div className="flex items-center justify-center space-x-2">
                                        <Link to={`/tasks/${item.id}/edit`} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700" title="Düzenle">
                                            <Icon name="edit" className="w-5 h-5" />
                                        </Link>
                                        <button onClick={() => handleDelete(item.id)} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700" title="Sil">
                ...
                {filteredTasks.length === 0 && <p className="text-center text-gray-500 py-8">{searchQuery ? 'Arama kriterlerine uygun görev bulunamadı.' : 'Gösterilecek görev bulunamadı.'}</p>}
            </div>
            
            {totalPages > 1 && (
                <div className="flex justify-between items-center mt-4 px-2">
                    <span className="text-sm text-gray-600 dark:text-gray-400">
                        Toplam {filteredTasks.length} kayıttan {filteredTasks.length > 0 ? indexOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, filteredTasks.length)} arası gösteriliyor.
                    </span>
