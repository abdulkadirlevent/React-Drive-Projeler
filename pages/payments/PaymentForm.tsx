...
const PaymentForm: React.FC<PaymentFormProps> = ({ initialData, onSave, onCancel }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <SelectField name="status" label="Durum" value={formData.status} onChange={handleChange} options={Object.values(PaymentStatus)} isEnum={true} required />
...
const SelectField: React.FC<any> = ({ name, label, options, isEnum, ...props }) => (
     <div>
        <label htmlFor={name} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{label}</label>
